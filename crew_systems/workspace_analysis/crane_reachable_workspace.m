...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
% now do the final crane version
craneDH = [
    0            0              0.3             0;
    0           -pi/2           0.0             0;
    0            pi/2           1.4             0;
...
% Reset DH 
craneDH_mod = craneDH;
craneDH_mod(2, 4) = crane_limits(2,1); % Set shoulder pitch to max reach
craneDH_mod(3, 3) = craneDH(3, 3) + max_extension; % Set arm to max distance
craneDH_mod(4,4) = crane_limits(4,2); % Set arm to max distance

count = 1;
for i=crane_limits(4,1):0.1:crane_limits(4,2)
    craneDH_mod(4,4) = i; % Set arm to max distance
    % Get shoulder YAW reachable workspace
    for theta=crane_limits(1,1):0.001:crane_limits(1,2)
        % Get the DH transformation matricies
        craneDH_mod(1, 4) = theta;
...
% Do shoulder stuff now %
% Reset DH params
craneDH_mod = craneDH;

% starting at full extension and pointing straigt up
craneDH_mod(3,3) = craneDH(3,3) + max_extension;
craneDH_mod(4,4) = crane_limits(4,2); % fully extend elbow to start

count = 1;
% Move shoulder to limits
for theta=crane_limits(2,2):-0.001:crane_limits(2,1)
    % Get the DH transformation matricies
    craneDH_mod(2, 4) = theta;
    T_final = pre_mult*SslModifDhTableToTransf(0, 5, craneDH_mod);
...
% Swing elbow
for theta=crane_limits(4,2):-0.001:crane_limits(4,1)
    % Get the DH transformation matricies
    craneDH_mod(4, 4) = theta;
    T_final = pre_mult*SslModifDhTableToTransf(0, 5, craneDH_mod);
...
% Swing elbow back to start
for theta=crane_limits(4,1):0.001:crane_limits(4,2)
    % Get the DH transformation matricies
    craneDH_mod(4, 4) = theta;
    T_final = pre_mult*SslModifDhTableToTransf(0, 5, craneDH_mod);
...
clear d max_extension T_final theta craneDH_mod count file_name